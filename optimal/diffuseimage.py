...
def one_sim(st, gamma_path, sample_point):
    
    problem = DiffuseImage(gamma_path, ini_cond, device)
    ...
    data_path = os.path.dirname(gamma_path)
    data_list = [d for d in sorted(os.listdir(data_path)) if d[-2:] == "h5"]
    state = []
...
if sample_style == 'random':
    select_index = [i for i in np.random.choice(np.arange(n_points), 50)]
